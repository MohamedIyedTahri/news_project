...
def _running_in_container() -> bool:
    """Return True when the process runs inside a containerised environment."""
...
def _resolve_bootstrap(bootstrap: str) -> str:
    ...
    From the host network that hostname often does NOT resolve. For host-based
    scripts we attempt a best-effort fallback to the mapped listener so local
    execution keeps working without extra configuration, while containers stick
    with the internal endpoint.
    """
    try:
        host, port = bootstrap.split(":", 1)
    except ValueError:
        return bootstrap
    if host == "kafka" and port == "9092" and not _running_in_container():
        alt = os.environ.get("KAFKA_BOOTSTRAP_FALLBACK", "localhost:29092")
        logger.info(
            "Bootstrap '%s' not resolvable from host context; using fallback '%s'. "
            "Set KAFKA_BOOTSTRAP_SERVERS to override.",
            bootstrap,
            alt,
        )
        return alt
